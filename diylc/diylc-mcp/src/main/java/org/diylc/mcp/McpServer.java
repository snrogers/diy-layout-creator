...
public class McpServer {
  ...
  public McpServer(ToolRegistry registry, PrintStream protocolOut) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    DiylcEngine engine = new DiylcEngine();
    ToolRegistry registry = new ToolRegistry(engine);
    System.err.println("[diylc-mcp] ready with " + registry.all().size() + " tools.");
    new McpServer(registry, protocolOut).run();
  }
  ...
  private ObjectNode toolsCallResult(JsonNode params) {
    ...
    try {
      Object raw = tool.handler().run(arguments);
