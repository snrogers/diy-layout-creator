...
- **diylc-mcp** — a [Model Context Protocol](https://modelcontextprotocol.io) server (`org.diylc.mcp.McpServer`) exposing the engine to AI agents over stdio JSON-RPC. Tools are declared in `ToolRegistry`; `DiylcEngine` wraps the `Presenter`, reusing the GUI's slot+click flow for edits so behaviour matches the app. Two modes: **headless** (default — `DummyView`, no GUI) and **headed** (`-Dorg.diylc.mcp.headed=true` — drives a live `MainFrame`'s Presenter for human+agent co-editing; each tool call runs on the Swing EDT via `EdtExecutor`). Depends on `core`+`library`+`swing`. See `diylc/diylc-mcp/README.md`. Build: `mvn -pl diylc-mcp -am package` → runnable `diylc-mcp/target/diylc-mcp.jar`.
