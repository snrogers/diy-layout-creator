...
- **diylc-mcp** — a headless [Model Context Protocol](https://modelcontextprotocol.io) server (`org.diylc.mcp.McpServer`) exposing the engine to AI agents over stdio JSON-RPC. It drives the same `Presenter` with a `DummyView` (no GUI), reusing the GUI's slot+click flow for edits so behaviour matches the app. Tools are declared in `ToolRegistry`; `DiylcEngine` wraps the `Presenter`. See `diylc/diylc-mcp/README.md`. Build: `mvn -pl diylc-mcp -am package` → runnable `diylc-mcp/target/diylc-mcp.jar`.
